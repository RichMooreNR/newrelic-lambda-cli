...
def _check_for_ingest_stack(session, resource_prefix=None):
    return _get_cf_stack_status(session, get_resource_name(INGEST_STACK_NAME, resource_prefix=resource_prefix))
...
def _create_role(input):
    ...
    stack_name = get_resource_name('NewRelicLambdaIntegrationRole', input.nr_account_id, input.resource_prefix)
...
def _import_log_ingestion_function(
    session, nr_license_key, enable_logs, memory_size, timeout, role_name, tags, resource_prefix
):
    ...
    with open(template_path) as template:
        change_set_name = "%s-IMPORT-%d" % (get_resource_name(INGEST_STACK_NAME, resource_prefix=resource_prefix), int(time.time()))
        click.echo("Creating change set: %s" % change_set_name)

        change_set = cf_client.create_change_set(
            StackName=get_resource_name(INGEST_STACK_NAME, resource_prefix=resource_prefix),
            TemplateBody=template.read(),
        ...
        _exec_change_set(cf_client, change_set, "IMPORT", get_resource_name(INGEST_STACK_NAME, resource_prefix=resource_prefix))
...
def _create_log_ingestion_function(
    ...
    change_set_name = "%s-%s-%d" % (get_resource_name(INGEST_STACK_NAME, resource_prefix=input.resource_prefix), mode, int(time.time()))
    click.echo("Creating change set: %s" % change_set_name)

    change_set = client.create_change_set(
        StackName=get_resource_name(INGEST_STACK_NAME, resource_prefix=input.resource_prefix),
    ...
    _exec_change_set(client, change_set, mode, get_resource_name(INGEST_STACK_NAME, resource_prefix=input.resource_prefix))
...
def update_log_ingestion_function(input):
    ...
    resources = client.describe_stack_resources(
        StackName=get_resource_name(INGEST_STACK_NAME, resource_prefix=input.resource_prefix), LogicalResourceId="NewRelicLogIngestion"
    )
    ...
    if (
        ...
        old_props = lambda_client.get_function_configuration(
            FunctionName=get_resource_name("newrelic-log-ingestion", resource_prefix=input.resource_prefix)
        )
        ...
        # ingest function
        client.delete_stack(StackName=get_resource_name(INGEST_STACK_NAME, resource_prefix=input.resource_prefix))
        client.get_waiter("stack_delete_complete").wait(StackName=get_resource_name(INGEST_STACK_NAME, resource_prefix=input.resource_prefix))
...
def remove_log_ingestion_function(input):
    ...
    if stack_status is None:
        ...
        return
    click.echo("Deleting New Relic log ingestion stack '%s'" % get_resource_name(INGEST_STACK_NAME, resource_prefix=input.resource_prefix))
    client.delete_stack(StackName=get_resource_name(INGEST_STACK_NAME, resource_prefix=input.resource_prefix))
    click.echo(
...
def auto_install_license_key(input):
    ...
    """
    assert isinstance(input, (IntegrationInstall, IntegrationUpdate))
    lk_stack_status = _get_cf_stack_status(input.session, get_resource_name(LICENSE_KEY_STACK_NAME, resource_prefix=input.resource_prefix))
...
def install_license_key(input, nr_license_key, policy_name=None, mode="CREATE"):
    ...
    if lk_stack_status is None:
        click.echo(
            ...
            % (get_resource_name(LICENSE_KEY_STACK_NAME, resource_prefix=input.resource_prefix), input.session.region_name)
        )
        try:
            client = input.session.client("cloudformation")
            ...
            change_set_name = "%s-%s-%d" % (
                get_resource_name(LICENSE_KEY_STACK_NAME, resource_prefix=input.resource_prefix),
                mode,
                int(time.time()),
            )
            click.echo("Creating change set: %s" % change_set_name)
            ...
            with open(template_path) as template:
                change_set = client.create_change_set(
                    StackName=get_resource_name(LICENSE_KEY_STACK_NAME, resource_prefix=input.resource_prefix),
                    TemplateBody=template.read(),
                    Parameters=parameters,
                    Capabilities=["CAPABILITY_NAMED_IAM"],
                    Tags=[{"Key": key, "Value": value} for key, value in input.tags]
                ...
                _exec_change_set(
                    client, change_set, mode, stack_name=get_resource_name(LICENSE_KEY_STACK_NAME, resource_prefix=input.resource_prefix)
                )
        except Exception as e:
            failure("Failed to create %s stack: %s" % (get_resource_name(LICENSE_KEY_STACK_NAME, resource_prefix=input.resource_prefix), e))
            return False
    return True
...
def remove_license_key(input):
    ...
    client = input.session.client("cloudformation")
    stack_status = _get_cf_stack_status(input.session, get_resource_name(LICENSE_KEY_STACK_NAME, resource_prefix=input.resource_prefix))
    if stack_status is None:
        click.echo(
        ...
        return
    click.echo("Deleting stack '%s'" % get_resource_name(LICENSE_KEY_STACK_NAME, resource_prefix=input.resource_prefix))
    client.delete_stack(StackName=get_resource_name(LICENSE_KEY_STACK_NAME, resource_prefix=input.resource_prefix))
    click.echo(
        "Waiting for stack deletion to complete, this may take a minute... ", nl=False
    )
    client.get_waiter("stack_delete_complete").wait(StackName=get_resource_name(LICENSE_KEY_STACK_NAME, resource_prefix=input.resource_prefix))
    success("Done")
...
def _get_license_key_policy_arn(session):
    ...
    try:
        stacks = client.describe_stacks(StackName=get_resource_name(LICENSE_KEY_STACK_NAME, resource_prefix=input.resource_prefix)).get(
            "Stacks", []
        )
    except botocore.exceptions.ClientError as e:
        if (
